...
def validate_candidate(reference, candidate, patient_ids, fino_ids):
...
def build_reembedded_bank(source, output, report, fino_path, minilm_encoder, biomedical_encoder, expected_source_sha):
    ...
    validation = validate_candidate(reference, candidate, patient_ids, fino_patient_ids(fino_path))
...
def main(argv=None):
    ...
    minilm = SentenceTransformer(MINILM_MODEL, revision=MINILM_REVISION, device=args["device"], local_files_only=True)
    biomedical = SentenceTransformer(BIOMED_MODEL, revision=BIOMED_REVISION, device=args["device"], local_files_only=True)
