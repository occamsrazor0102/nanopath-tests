...
class DinoV2ViT(nn.Module):
    ...
    def forward(self, x, masks=None, checkpoint=False, feature_blocks=None):
        x = self._prepare_tokens(x, masks)
        selected = {}
        requested = None if feature_blocks is None else tuple(int(i) for i in feature_blocks)
        if requested is not None:
            if len(requested) == 0 or len(set(requested)) != len(requested):
                raise ValueError("feature_blocks must be non-empty and unique")
            if any(i < 0 or i >= len(self.blocks) for i in requested):
                raise ValueError("feature_blocks contains an invalid transformer block")
        for index, block in enumerate(self.blocks):
            x = torch.utils.checkpoint.checkpoint(block, x, use_reentrant=False) if checkpoint and self.training else block(x)
            if requested is not None and index in requested:
                selected[index] = self.norm(x)[:, 0]
        result = {
            "x_norm_clstoken": self.norm(x)[:, 0],
            "x_norm_regtokens": self.norm(x)[:, 1 : 1 + self.registers],
            "x_norm_patchtokens": self.norm(x)[:, 1 + self.registers :],
        }
        if requested is not None:
            result["x_norm_probe_features"] = torch.cat([selected[index] for index in requested], dim=-1)
        return result
